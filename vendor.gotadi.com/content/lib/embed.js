var GTD_LOGIN_API = "https://api.gotadi.com/embed/api/embed/partner/login";
var REQUESTER_NAME = 'X-ibe-req-name';
var DEFAULT_REQUEST_BODY = JSON.stringify({
    "user_id": "anonymous",
    "layout": "dual"
});
var GOTADI_IFRAME_ID = 'gotadi-iframe-container';
var GOTADI_IFRAME_PARTNER_CODE = 'partner_code';

window.addEventListener('load', function() {
    loadGtdPartnerIframe();
}, false);

function loadGtdPartnerIframe() {
    // default
    var gotadi_ambed = document.getElementById(GOTADI_IFRAME_ID);
    var xmlhttp = new XMLHttpRequest(); // new HttpRequest instance
    xmlhttp.open("POST", GTD_LOGIN_API);
    xmlhttp.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
    // get partner info
    xmlhttp.setRequestHeader(REQUESTER_NAME, gotadi_ambed.getAttribute('partner_code'));
    ...
    xmlhttp.onload = function() {
        ...
        ifrm.setAttribute("src", jsonResponse.redirect_url);
        if (gotadi_ambed !== null) {
            gotadi_ambed.appendChild(ifrm);
            iFrameResize({
                autoResize: true
            });
        }
    };
    xmlhttp.send(DEFAULT_REQUEST_BODY);
}
